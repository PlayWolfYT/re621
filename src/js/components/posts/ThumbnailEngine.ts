...
export default class ThumbnailEngine extends Component {
  ...
  public Settings = {
    ...
    imageWidth: 250,
    imageRatio: 1,
    loadMethod: ImageLoadMethod.Sample,
    crop: false,
  ...
  public async create() {
    ...
    this.on(
      "settings.imageWidth settings.imageRatio settings.crop settings.highlightVisited settings.hideInfoBar settings.colorFavCount",
      () => {
        this.updateContentHeader();
      }
    );
  ...
  public updateVisibility() {
    for (const thumb of this.thumbnails) thumb.updateVisibility();
  }
