...
export default class HeaderCustomizer extends Component {
  ...
  private createConfigInterface(): void {
    ...
    const newTabForm = new Form(
      ...
      [
        ...
        Form.checkbox({
          label: "Attach to the right side",
          value: false,
          name: "right",
        }),
        ...
        Form.div({
          value:
            "Drag-and-drop tabs to re-arrange.<br />Click on a tab to edit it.",
        }),
      ],
      (values, form) => {
        this.addTab({
    ...
    this.addTabModal.getElement().on("dialogclose", () => {
      this.isEditing = false;
    });
  ...
  public rebuildTabs(): void {
    this.$menu.html("");
    for (const value of this.Settings.tabs) this.createTabElement(value);
    this.reloadTabMargins();
  }
  ...
  private createTabElement(
    ...
    if (config.href !== "")
      $link.attr("href", encodeURI(this.processTabVariables(config.href)));

    const compareHREF = config.href.toLowerCase();
    if (compareHREF === "/forum_topics" && this.hasForumUpdates)
      $link.addClass("tab-has-updates");
    else if (
      (compareHREF == "/users/%userid%" || compareHREF == "/users/home") &&
      !User.loggedIn
    ) {
      $link.attr("href", "/session/new").addClass("tab-login").html("Sign In");
    }
    ...
    if (
      config.href.trim() !== "" &&
      Page.getURL().pathname.includes(
        this.processTabVariables(config.href).split("?")[0]
      )
    ) {
  ...
  private reloadTabMargins(): void {
    ...
    this.$menu.find("li[align=true]").first().addClass("margined");
  }
}
